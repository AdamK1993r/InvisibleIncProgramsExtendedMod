...
local W93_combatAI_delay =
{
	name = STRINGS.PROGEXTEND.UI.AI_DELAY,
        proxy = true,
	...
	onTooltip = function( self, hud, sim, abilityOwner, abilityUser, targetUnitID )
		...
		section:addAbility( STRINGS.PROGEXTEND.UI.AI_DELAY, string.format(STRINGS.PROGEXTEND.UI.AI_DELAY_DESC, math.max(abilityOwner:getSkillLevel( "hacking" ) - 2, 0)), self.profile_icon )
		section:addAbility( STRINGS.PROGEXTEND.UI.AI_SKILL, STRINGS.PROGEXTEND.UI.AI_SKILL_DELAY, "gui/icons/skills_icons/skills_icon_small/icon-item_analyze_small.png" )
		if reason then
	...
	profile_icon = "gui/icons/action_icons/Action_icon_Small/icon-action_lock_small.png",
	...
	canUseAbility = function( self, sim, abilityOwner, unit, targetUnitID )
		...
		if (abilityOwner:getSkillLevel( "hacking" ) or 0) < 2 then
			return false, string.format( STRINGS.UI.TOOLTIP_REQUIRES_SKILL_LVL, "HACKING", 2 )
		end

		if sim:getPC():getCpus() < 1 then
	...
	executeAbility = function( self, sim, abilityOwner, unit, targetUnitID )
		...
		sim:dispatchEvent( simdefs.EV_UNIT_USECOMP, { unitID = unit:getID(), targetID = targetUnit:getID(), facing = facing, sound = "SpySociety/Actions/monst3r_jackin" , soundFrame = 16, useTinker=true } )
            	sim:triggerEvent(simdefs.TRG_UNIT_HIJACKED, { unit = targetUnit, sourceUnit = unit, action = "delay" } )
		sim:getPC():addCPUs( -1, sim, x1, y1 )
		sim:dispatchEvent( simdefs.EV_WAIT_DELAY, 60*0.65)
		targetUnit:processEMP( 5 - (math.max(abilityOwner:getSkillLevel( "hacking" ) - 2,0)) )
		sim:processReactions( abilityOwner )
		sim:dispatchEvent( simdefs.EV_UNIT_REFRESH, { unit = targetUnit } )
	end
}

return W93_combatAI_delay