...
local W93_combatAI_scan =
{
	name = STRINGS.PROGEXTEND.UI.AI_SCAN,
        proxy = true,
	...
	onTooltip = function( self, hud, sim, abilityOwner, abilityUser, targetUnitID )
		...
		section:addAbility( STRINGS.PROGEXTEND.UI.AI_SCAN, STRINGS.PROGEXTEND.UI.AI_SCAN_DESC, self.profile_icon )
		if reason then
	...
	profile_icon = "gui/icons/skills_icons/skills_icon_small/icon-item_scan_small.png",
	...
	acquireTargets = function( self, targets, game, sim, abilityOwner, unit )
		...
		for _, targetUnit in pairs(sim:getAllUnits()) do
			...
			if x1 and self:isTarget( abilityOwner, unit, targetUnit ) and sim:getNPC():findMainframeAbility("W93_AI_assembly") and sim:getNPC():hasMainframeAbility("W93_AI_assembly") and not sim:getNPC():getTraits().showAITemp then
	...
	executeAbility = function( self, sim, abilityOwner, unit, targetUnitID )
		...
		sim:triggerEvent(simdefs.TRG_UNIT_HIJACKED, { unit = targetUnit, sourceUnit = unit, action = "scan" } )
		sim:getPC():addCPUs( -1, sim, x1, y1 )
		sim:getPC():unlockAllMainframe()
		sim:processReactions( abilityOwner )
		sim:dispatchEvent( simdefs.EV_UNIT_REFRESH, { unit = targetUnit } )
	end
}

return W93_combatAI_scan