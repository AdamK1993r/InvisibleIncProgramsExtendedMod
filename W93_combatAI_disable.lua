...
local W93_combatAI_disable =
{
	name = STRINGS.PROGEXTEND.UI.AI_DISABLE,
        proxy = true,
	...
	onTooltip = function( self, hud, sim, abilityOwner, abilityUser, targetUnitID )
		...
		section:addAbility( STRINGS.PROGEXTEND.UI.AI_DISABLE, STRINGS.PROGEXTEND.UI.AI_DISABLE_DESC, "gui/icons/thought_icons/status_action.png" )
		if reason then
	...
	acquireTargets = function( self, targets, game, sim, abilityOwner, unit )
		...
		for _, targetUnit in pairs(sim:getAllUnits()) do
			...
			if x1 and self:isTarget( abilityOwner, unit, targetUnit ) and sim:getNPC():findMainframeAbility("W93_AI_assembly") and sim:getNPC():hasMainframeAbility("W93_AI_assembly") and sim:getNPC():getTraits().hasAIroutine then
	...
	canUseAbility = function( self, sim, abilityOwner, unit, targetUnitID )
		...
		if (abilityOwner:getSkillLevel( "hacking" ) or 0) < 5 then
			return false, string.format( STRINGS.UI.TOOLTIP_REQUIRES_SKILL_LVL, "HACKING", 5 )
		end

		if sim:getPC():getCpus() < 10 then
	...
	executeAbility = function( self, sim, abilityOwner, unit, targetUnitID )
		...
		sim:dispatchEvent( simdefs.EV_UNIT_USECOMP, { unitID = unit:getID(), targetID = targetUnit:getID(), facing = facing, sound = "SpySociety/Actions/monst3r_jackin" , soundFrame = 16, useTinker=true } )
            	sim:triggerEvent(simdefs.TRG_UNIT_HIJACKED, { unit = targetUnit, sourceUnit = unit, action = "remove" } )
		sim:getPC():addCPUs( -10, sim, x1, y1 )
